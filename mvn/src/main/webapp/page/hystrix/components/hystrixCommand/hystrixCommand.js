...
(function(window) {
	...
	var playlistErrorCount=0;
	var collectionErrorCount=0;
	var commandCmsRenderErrorCount=0;
	var channelErrorCount=0;
	var fantuanErrorCount=0;

	// cache the templates we use on this page as global variables (asynchronously)
	jQuery.get(getRelativePath("../page/hystrix/components/hystrixCommand/templates/hystrixCircuit.html"), function(data) {
		hystrixTemplateCircuit = data;
	});
	jQuery.get(getRelativePath("../page/hystrix/components/hystrixCommand/templates/hystrixCircuitContainer.html"), function(data) {
				...
				if(data && data.type == 'HystrixCommand') {
					if (data.deleteData == 'true') {
						...
						if(data.name=='video'){
							...
							videoErrorCount+=data.errorCount;
							$("#videoErrorCount").html(videoErrorCount);
							$("#videoErrorPercentage").html(data.errorPercentage);
						}
						if(data.name=='playlist'){
							playlistErrorCount+=data.errorCount;
							$("#playlistErrorCount").html(playlistErrorCount);
							$("#playlistErrorPercentage").html(data.errorPercentage);
						}
						if(data.name=='collection'){
							collectionErrorCount+=data.errorCount;
							$("#collectionErrorCount").html(collectionErrorCount);
							$("#collectionErrorPercentage").html(data.errorPercentage);
						}
						if(data.name=='fantuan'){
							fantuanErrorCount+=data.errorCount;
							$("#fantuanErrorCount").html(fantuanErrorCount);
							$("#fantuanErrorPercentage").html(data.errorPercentage);
						}
					}
				}
			}
